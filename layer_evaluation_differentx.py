...
parser.add_argument('--num_heads', default=scale, type=int, help='number of heads for multi-headed attention (default: 8)')
parser.add_argument('--depth', default=scale, type=int, help='depth of the transformer architecture (default: 12)')
parser.add_argument('--embed_dim', default=scale*32, type=int, help='embedding dimension of the transformer feature extractor (default: 256)')
...
parser.add_argument('--a', default=2, type=int, help='1/a^b, 1/a^c')
parser.add_argument('--b', default=2, type=int, help='1/a^b')
parser.add_argument('--c', default=2, type=int, help='1/a^c')
...
if 1:
    ...
    if args.exp_name == 'Regular4_delta':
        ...
        setting_hypers = 'smw=1@'+str(args.a)+'^'+str(args.b)+'_sxy=1@'+str(args.a)+'^'+str(args.c)+'_depth='+str(args.depth)+'_dim='+str(args.embed_dim)+'_heads='+str(args.num_heads)
        optimer_hypers = 'lr='+str(args.lr)+'_wd='+str(args.wd)+'_BS='+str(args.batch_size)+'_Step='+str(args.steps)+'_EP='+str(args.epochs)
        
        folder = '../data/trained_models/'+setting_hypers+'/'+optimer_hypers+'/'
        prior = Regular4(sm=sm, sw=sw, sx=sx, sy=sy)
...
if 1: #explore train visualizer
    epoch = 12
    state_folder = folder+'checkpoint/EP='+str(epoch)+'.pth'
    model = TransformerModel(n_dims=priorProcesser.d, n_positions=args.num_k,
                             n_embd=args.embed_dim, n_layer=args.depth,
                             n_head=args.num_heads, special_dimension=True)
    model.cuda()
    ...
    selected_k = [1,3,4,8,16,32]
    selected_k = [k-1 for k in selected_k]
    
    with torch.no_grad():
        print('check basic performance')
        new_task_m = priorProcesser.topic_ms[1]
        ...
        bs_label1 = bs_xs@priorProcesser.topic_ws[1] 
        bs_label2 = bs_xs@priorProcesser.topic_ws[2] 
        bs_label3 = bs_xs@priorProcesser.topic_ws[3] 
        results1 = model.forward3(torch.from_numpy(bs_xs).cuda().float(),
                                 torch.from_numpy(bs_label1).cuda().float())
        results2 = model.forward3(torch.from_numpy(bs_xs).cuda().float(),
                                 torch.from_numpy(bs_label2).cuda().float())
        results3 = model.forward3(torch.from_numpy(bs_xs).cuda().float(),
                                 torch.from_numpy(bs_label3).cuda().float())
        
        print('performance of task 1', (np.mean((results1['prediction'].cpu().numpy()-bs_label1)**2, axis=0)**0.5)[selected_k])
        print('performance of task 2', (np.mean((results2['prediction'].cpu().numpy()-bs_label2)**2, axis=0)**0.5)[selected_k])
        print('performance of task 3', (np.mean((results3['prediction'].cpu().numpy()-bs_label3)**2, axis=0)**0.5)[selected_k])
        '''
        prediction1 = results['prediction'].cpu().float().numpy()
        print(np.mean((prediction1-bs_learning)**2, axis=0)[selected_k])
        hiddens1 = results['hiddens']
        bs_task1 = bs_xs@priorProcesser.topic_ws[1] 
        print('1 to 1 ', (np.mean((prediction1-bs_task1)**2, axis=0)**0.5)[selected_k])
        bs_task2 = bs_xs@priorProcesser.topic_ws[2] 
        print('1 to 2 ', (np.mean((prediction1-bs_task2)**2, axis=0)**0.5)[selected_k])
        bs_task3 = bs_xs@priorProcesser.topic_ws[3] 
        print('1 to 3 ', (np.mean((prediction1-bs_task3)**2, axis=0)**0.5)[selected_k])
        '''
    '''   
        new_task_m = priorProcesser.topic_ms[2]*0
        ...
        new_task_3 = priorProcesser.topic_ms[3]*0
